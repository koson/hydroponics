      ...
      if (mySerial.available())
    {
      ...
      else // End of Line
      {
        ...
        {   
          ...
          feed_Read = (text_data[8]-48)*1000 + (text_data[9]-48)*100 + (text_data[10]-48)*10 + (text_data[11]-48);
        }
   ...
   if(WiFi.status() == WL_CONNECTED)
    {
      if (client.connect("BearingHydroponics", mqtt_user, mqtt_password))
      {
        if (!isnan(pH_Read) && !isnan(Temp_Read))
        {
          ...
          client.publish("/bearinghydro/pH_crop1", pH_water);
          client.publish("/bearinghydro/temp_crop1", temp_water);
          client.publish("/bearinghydro/feed_crop1", feed_ph);
