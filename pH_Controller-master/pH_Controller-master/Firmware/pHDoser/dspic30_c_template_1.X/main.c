...
void pH_Read()
{
	...
	for(k=1;k<=10000;k++)  // Low pass
	{
		...
		pH_read = ((temp2/10000) * pHCoeff)+pH_Compensate;	
...
void pH_Control()
{
			...
			else if( (pH_read*10) > (pH_HighSP*10) && (pH_flag == 1) )
			{
				RY1 = 1; // Relay ON
				pH_flag = 0;
                pH_FeedCount = pH_FeedCount+((float)ph_dose_time * 8.33); // pump feed @ 500ml/min = 8.33 ml/sec
			}
			else if((pH_read*10) <= (pH_LowSP*10)) // Lower limit alarm system
			{
				RY1 = 0; // Relay OFF
				pH_flag = 1;
			}
...
int main(void)
{
    ...
    // Initial
     sprintf(buf,"Bearing Hydroponics"); 
     LCDPrintxy(1,1,0, buf);
     sprintf(buf," pHIoT  V.1.4 "); 
     LCDPrintxy(1,2,0, buf);      // Display second
     __delay_ms(2000);

     sprintf(buf,"Firmware ver.1.4 "); 
     LCDPrintxy(1,1,0, buf);
     sprintf(buf,"Update  29/03/17  "); 
     LCDPrintxy(1,2,0, buf);      // Display second
     __delay_ms(2000);
    ...
    // Show data for calibration
     ...
     LCDPrintxy(1,1,0, buf);
     sprintf(buf,"SP=%.1f m=%f",pH_SP,pHCoeff); 
     LCDPrintxy(1,2,0, buf);      // Display second
     __delay_ms(2000);
...
int aa_test=0;
    ...
    while(1)
    {
        ...
        //if(pH_read*10 < 10 && (int)temperature_read > 9)
       // {
       ...
       // }
        ...
        if(POSCNT >=0 && POSCNT <= 3) 
        {
            ...
            /*if(alarm_mode == 1)  // pH Alarm
            {
                sprintf(buf,">>> pH = %1.1f <<<",pH_read); 
                LCDPrintxy(1,1,0, buf);
                sprintf(buf,"Lower than limit"); 
                LCDPrintxy(1,2,0, buf);      // Display second
                delay_ms(200);
