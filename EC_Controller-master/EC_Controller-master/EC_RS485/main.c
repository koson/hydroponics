...
unsigned int pH4_cal;
unsigned int pH7_cal;
...
void __attribute__((interrupt, no_auto_psv)) _U1RXInterrupt(void)
{
	...
	if(rx_data == 'j')
	{
        ...
        if(packet[0] == ph_address && packet[1] == 'S' && packet[2] == 'j')
        {
            pH_SP = (packet[2]-48) + (packet[3]-48)*0.1; 
            Eeprom_WriteWord(0,(int)(pH_SP*10));
            __delay_ms(10);   
        }
        else if(packet[0] == ph_address && packet[1] == 'L' && packet[2] == 'j')
        {
            pH_LowSP = (packet[2]-48) + (packet[3]-48)*0.1; 
            Eeprom_WriteWord(1,(int)(pH_LowSP*100));
            __delay_ms(10);    
        }
        else if(packet[0] == ph_address && packet[1] == 'H' && packet[2] == 'j')
        {
            pH_HighSP = (packet[2]-48) + (packet[3]-48)*0.1; 
            Eeprom_WriteWord(2,(int)(pH_HighSP*100));
            __delay_ms(10);  
        }
        else if(packet[0] == ph_address && packet[1] == 'D' && packet[2] == 'j')
        {
            ph_dose_time = (packet[2]-48)*10 + (packet[3]-48); 
            Eeprom_WriteWord(5,(int)(ph_dose_time));            
            __delay_ms(10);     
        }
        else if(packet[0] == ph_address && packet[1] == 'I' && packet[2] == 'j')
        {
            pH_TimeInterval = (packet[2]-48)*10 + (packet[3]-48); 
            Eeprom_WriteWord(6,(int)(pH_TimeInterval));            
            __delay_ms(10);      
        }
        else if(packet[0] == ph_address && packet[1] == 'R' && packet[2] == 'j')
        {
            //DIR_RS485 = 1; // Send
            while(DataRdyUART1());
            sprintf(bufUART1,"cet%.1fjab",pH_read);
            putsUART1(bufUART1);
            __delay_ms(230);  
            //DIR_RS485 = 0;
        }
        else if(packet[0] == ph_address && packet[1] == 'F' && packet[2] == 'j')
        {
            //DIR_RS485 = 1; // Send
            while(DataRdyUART1());
            sprintf(bufUART1,"ecf%04djtq",(int)pH_FeedCount);
            putsUART1(bufUART1);
            __delay_ms(230);
            //DIR_RS485 = 0;
        }
        else if(packet[0] == ph_address && packet[1] == 'T' && packet[2] == 'j')
        {
            //DIR_RS485 = 1; // Send
            while(DataRdyUART1());
            sprintf(bufUART1,"ect%02djyv",(int)temperature_read);
            putsUART1(bufUART1);
            __delay_ms(230);  
            //DIR_RS485 = 0;
        }
...
void pH_Read()
{
	...
	for(k=1;k<=5000;k++)  // Low pass
	{
		...
		pH_read = (pH_ADC * 0.02234375) -4.5804;	    
...
void feed_acid()
{
if(pH_read > 0.0f && pH_read < 255.0f)
	RY1 = 0;
}
void stop_acid()
{
...
void pH_Control()
{
		if((pH_read*10) == (pH_SP*10))
		{
			RY1 = 0; // Relay OFF
		}
		else if(((pH_read*10) < (pH_SP*10)) )
		{
			RY1 = 1; // Relay ON
...
void pH_Control()
{
      ...
      else if( (pH_read*10) < (pH_SP*10) && (pH_flag == 0) )
      {
        RY1 = 1; // Relay ON
        pH_FeedCount = pH_FeedCount+((float)ph_dose_time*2.0833333333333333333333333333333); // pump feed @ 500ml/min = 8.33 ml/sec
        
      }
      else if( (pH_read*10) < (pH_LowSP*10) && (pH_flag == 1) )
      {
        RY1 = 1; // Relay ON
        pH_flag = 0;
        pH_FeedCount = pH_FeedCount+((float)ph_dose_time*2.0833333333333333333333333333333); // pump feed @ 500ml/min = 8.33 ml/sec
...
int main(void)
{
    ...
    __delay_ms(10);
    EC_SP = Eeprom_ReadWord(7);		
    EC_SP = (float)EC_SP/10;
    __delay_ms(10);
    EC_LowSP = Eeprom_ReadWord(8);	
    EC_LowSP = (float)EC_LowSP/100;
    __delay_ms(10);
    EC_HighSP = Eeprom_ReadWord(9);	
    EC_HighSP = (float)EC_HighSP/100;
    __delay_ms(10);
    EC_Cal1413 = Eeprom_ReadWord(10);		
    __delay_ms(10);
    ec_dose_time = Eeprom_ReadWord(11);
    __delay_ms(10);
    EC_TimeInterval = Eeprom_ReadWord(12);	
    __delay_ms(10);
    ...
    pH_TimeInterval = 1;
    ph_dose_time = 3;*/
    ec_compensate = 1.413 - (0.02234375*pH4_cal);
    //if(pH4_cal == -1)
    //    pH4_cal = 299;
    ...
    // Initial
     ...
     sprintf(buf," ECIoT  V.1.0   "); 
     LCDPrintxy(1,2,0, buf);      // Display second
     __delay_ms(1000);

     sprintf(buf,"Firmware ver.1.0 "); 
     LCDPrintxy(1,1,0, buf);
     sprintf(buf,"Update  14/09/17  "); 
     LCDPrintxy(1,2,0, buf);      // Display second
     __delay_ms(1000);

    // Show data for calibration
     sprintf(buf,"EC1413=%4d     ",pH4_cal); 
     LCDPrintxy(1,1,0, buf);
     sprintf(buf,"EC com=%1.1f     ",ec_compensate); 
     LCDPrintxy(1,2,0, buf);      // Display second
     __delay_ms(1000);
...
int aa_test=0;
    ...
    while(1)
    {
		...
		if(ph_minute == pH_TimeInterval && ph_second == ph_dose_time) 
	 	{	 
			...
			ph_second = 0;
		 	ph_minute = 0;
            //pH_FeedCount = pH_FeedCount/5;
	 	}
        //menu_count = POSCNT;

        // Send data to ESP8266
        sprintf(buf,"A%2.1fB%2dC%dD\r\n",pH_read,(int)temperature_read,(int)pH_FeedCount);
        putsUART1(buf);
        ...
        if(POSCNT >=20 && POSCNT <= 23)  // EC Interval Time
        {
           sprintf(buf,"   EC Interval  "); 
           LCDPrintxy(1,1,0, buf);
         ...
         if(POSCNT >=24 && POSCNT <= 27)  // EC 1.413 ms
         {
              timer1_reset();			
		      sprintf(buf,"  EC 1413  uS   "); 
		      LCDPrintxy(1,1,0, buf);
			  ...
			  int cal4_cnt;
               if(!ENC_SW)
              {       
                    for(cal4_cnt=10;cal4_cnt>0;cal4_cnt--)
                    {
                        __delay_ms(1000); 
                        sprintf(buf,"  EC 1413  uS   ");
                        LCDPrintxy(1,1,0, buf);
                    ...
                    ec_compensate = 1.413 - (0.02234375*pH_ADC);
       ...
       }//EC 1.413 ms Calibration
	
        if(POSCNT >=28 && POSCNT <= 31)  
        {
            debug_mode = 1;
	...
	{ 
      sprintf(buf," EC Feed count  "); 
      LCDPrintxy(1,1,0, buf);
	  sprintf(buf,"   %04d  ml     ",(int)pH_FeedCount);
	  LCDPrintxy(1,2,0, buf);      // Display second  
...
{		  
  ...
  ConfigIntUART1(UART_RX_INT_DIS &							// ENABLE RX Interrupt
